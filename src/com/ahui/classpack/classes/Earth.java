		...
		taskSave = Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(ClPk.getInstance(), new Runnable()
		{
			...
			public void run()
			{
				if(Earth.this.getPlayer()!=null)
				{
					...
					ClPk.playerClass.getConfig().set(p.getUniqueId()+".branch", Earth.this.getBranch());
					ClPk.playerClass.getConfig().set(p.getUniqueId()+".mana", Earth.this.getEnergy());
					ClPk.playerClass.getConfig().set(p.getUniqueId()+".level", Earth.this.getLevel());
					ClPk.playerClass.saveConfig();
					ClPk.playerClass.reloadConfig();
					saveAll();
				}
